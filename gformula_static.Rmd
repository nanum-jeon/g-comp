---
title: "Introduction to G-Methods: G-Formula (Static Setting)"
author: "Nanum Jeon"
date: "`r Sys.Date()`"
output:
  html_document:
...
Our variables are:
- $Z$: HIV viral load at baseline (0 = low, 1 = high)  
...
kable(treated_outcomes %>% select(z, y, prob, weighted_y),
      ...
      col.names = c("Z", "E(Y|A=1,Z)", "P(Z)", "Weighted Contribution"),
      digits = 4) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
kable(untreated_outcomes %>% select(z, y, prob, weighted_y),
      ...
      col.names = c("Z", "E(Y|A=0,Z)", "P(Z)", "Weighted Contribution"),
      digits = 4) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
a_model <- glm(a ~ z, family = binomial(), data = z_model_data)
...
# Show the data structure to understand confounding
data_summary <- data %>%
  group_by(z) %>%
  summarise(
    n_total = sum(n),
    prop_treated = sum(n[a == 1]) / sum(n),
    mean_outcome_treated = y[a == 1],
    mean_outcome_untreated = y[a == 0],
    diff_within_stratum = y[a == 1] - y[a == 0],
    .groups = 'drop'
  )

kable(data_summary,
      caption = "Table 8: Understanding Confounding Structure",
      col.names = c("Z", "Total N", "Prop. Treated", "Y|A=1", "Y|A=0", "Difference"),
      digits = 3) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

# Show why crude analysis is biased
cat("Why the crude analysis is biased:\n")
cat("- Higher viral load (Z=1) patients: more likely treated, worse baseline prognosis\n")
cat("- Lower viral load (Z=0) patients: less likely treated, better baseline prognosis\n")
cat("- This creates confounding that biases the crude estimate\n")
cat("- G-formula adjusts by standardizing over the confounder distribution\n")
```
